...
// f(x)的参数为a,b,c,d，其中x为当前要计算的点的横坐标,abcd为二维数组，k为当前x所取的abcd下标
function gdao(a, b, c, d, k, x) {
	var fenzi = 0;
	var fenmu = 0;
	for (var i = 0; i < a.length-1; i++) {
		var fi = a[i + 1][k] + b[i + 1][k] * (x) + c[i + 1][k] * (x * x)
				+ d[i + 1][k] * (x * x * x);
		var fjdao = 0
		for (j = 1; j <= i; j++) {
			fjdao += b[j][k] + 2 * c[j][k] * x + 3 * d[j][k] * (x * x);
		}
		var fidao = b[i + 1][k] + 2 * c[i + 1][k] * x + 3 * d[i + 1][k]
				* (x * x);
		fenzi += fi * fjdao + 1 / 2 * fidao * fi;
		fenmu += fi;
	}
	return -fenzi / fenmu;
}
...
function data5() {
	...
	// 计算g0
	var delta = (data[1].dataPoints[1].x - data[1].dataPoints[0].x)/10;
	for (var j = 0; j < data[1].dataPoints.length; j++) {
		if(j != 0){
			data[0].dataPoints[j].y = data[0].dataPoints[j-1].y;
		}
		for (var m = 0; m < 10; m++) {
			var x = data[1].dataPoints[j].x;
			data[0].dataPoints[j].y += gdao(a, b, c, d, Math.floor(x), x + delta
					* m)*delta;
		}
		data[0].dataPoints[j].y *= 0.001;
	}
	return data;
}
...
// 平滑+次序调整
function data6() {
	// 首先按照非零开始从早到晚排序，取前7个放在第7-13位，其余的分别放在0-6和14-19位这样子，在data2的基础上进行
	var data = data2();
	var first = new Array();
	for (var i = 0; i < data.length; i++) {
...
function data7() {
	...
	for (var i = 0; i < 20; i++) {
		...
		data[i].dataPoints[j * div - 1] = {
			x : j - 1,
			y : jsonData[i][j - 1],
			pd:data[i].dataPoints[j * div - 2].pd
		};
	...
	for (var j = 0; j < data[1].dataPoints.length; j++) {
		...
		for (var i = 0; i < data.length; i++) {
			for (var k = 1; k < i; k++) {
				data[0].dataPoints[j].y += data[k].dataPoints[j].pd;
			}
		}
		data[0].dataPoints[j].y /= (-data.length);
	}
